...
class Mailchimp {
    ...
    public function __construct($opts=array()) {
        ...
        if (!$this->CI->config->item('mc_apiurl')) {
            log_message('error', 'You must provide a MailChimp API URL.');
            show_error('You must provide a MailChimp API URL',500);
        }

        $this->root = $this->CI->config->item('mc_apiurl');

        if (!$this->CI->config->item('mc_apikey')) {
            log_message('error', 'You must provide a MailChimp API key.');
            show_error('You must provide a MailChimp API key',500);
        }
        ...
        if (is_null($this->CI->config->item('mc_debug'))) {
            ...
            $this->debug = $this->CI->config->item('mc_debug');
        }
    ...
    public function call($url, $params) {
        ...
        if(curl_error($ch)) {
            $response = array('status' => 'error',
                              ...
                              'error' => "API call to $url failed: " . curl_error($ch)
                              );
    ...
    public function castError($result) {
        if ($result['status'] !== 'error' || !$result['name']) {
            show_error('We received an unexpected error: ' . json_encode($result),500);
        }

        show_error($result['error'], $result['code']);
    }
